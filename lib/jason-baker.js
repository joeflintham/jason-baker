...
var jasonBaker = function(recipe, options, partytime) {
  ...
  var findStarRoot = function(starRoot, stars){
    console.log(starRoot, stars);
    if (stars instanceof Array){
      starRoot = stars.reduce(findStarRoot, starRoot);
    }
    
    var preStar = stars.split('*');
    console.log(preStar);
    var starRoot = preStar[0].split('/')[0];
    ...
    if (!recipe) {
      ...
      if (recipe.some(findStars)) {
        ...
        stars().then(function(recipes){
          var starRoot;
          if (options.starsToTiers) {
            starRoot = recipe.reduce(findStarRoot, null);
          }
          bake(recipes);
        })
